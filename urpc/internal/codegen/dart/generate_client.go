...
func generateProcedureImplementation(g *genkit.GenKit, sch schema.Schema) {
	...
	for _, procNode := range sch.GetProcNodes() {
		...
		g.Block(func() {
			...
			g.Break()
			g.Linef("%s withRetries(RetryConfig config) { retryConfig = RetryConfig.sanitised(config); return this; }", builderName)
			g.Break()
			g.Linef("%s withTimeout(TimeoutConfig config) { timeoutConfig = TimeoutConfig.sanitised(config); return this; }", builderName)
			g.Break()
...
func generateStreamImplementation(g *genkit.GenKit, sch schema.Schema) {
	...
	for _, streamNode := range sch.GetStreamNodes() {
		...
		g.Block(func() {
			...
			g.Linef("%s withHeader(String key, String value) { _headers[key] = value; return this; }", builderName)
			g.Break()
			g.Linef("%s withReconnect(ReconnectConfig config) { reconnectConfig = ReconnectConfig.sanitised(config); return this; }", builderName)
			g.Break()
