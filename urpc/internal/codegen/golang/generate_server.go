...
func generateServer(sch schema.Schema, config Config) (string, error) {
	...
	g.Line("//   - Handler dispatch (procedures vs streams)")
	...
	g.Line("//   http.HandleFunc(\"POST /api/v1/urpc\", func(w http.ResponseWriter, r *http.Request) {")
	g.Line("//       ctx := r.Context()")
	g.Line("//       ufoCtx := AppContext{DB: db, Foo: \"bar\"}")
	g.Line("//       httpAdapter := NewServerNetHTTPAdapter(w, r)")
	g.Line("//       server.HandleRequest(ctx, ufoCtx, httpAdapter)")
	g.Line("//   })")
