...
type procOrStreamMetaFormatter struct {
	g                 *genkit.GenKit
	procMeta          *ast.ProcOrStreamDeclChildMeta
	children          []*ast.ProcOrStreamDeclChildMetaChild
	maxIndex          int
	currentIndex      int
	currentIndexEOF   bool
	currentIndexChild ast.ProcOrStreamDeclChildMetaChild
}

func newProcOrStreamMetaFormatter(procMeta *ast.ProcOrStreamDeclChildMeta) *procOrStreamMetaFormatter {
	if procMeta == nil {
		procMeta = &ast.ProcOrStreamDeclChildMeta{}
	}

	if procMeta.Children == nil {
		procMeta.Children = []*ast.ProcOrStreamDeclChildMetaChild{}
	}
	...
	return &procOrStreamMetaFormatter{
...
func (f *procOrStreamMetaFormatter) loadNextChild() {
...
func (f *procOrStreamMetaFormatter) peekChild(offset int) (ast.ProcOrStreamDeclChildMetaChild, ast.LineDiff, bool) {
	peekIndex := f.currentIndex + offset
	peekIndexEOF := peekIndex < 0 || peekIndex > f.maxIndex
	peekIndexChild := ast.ProcOrStreamDeclChildMetaChild{}
...
func (f *procOrStreamMetaFormatter) LineAndComment(content string) {
...
func (f *procOrStreamMetaFormatter) LineAndCommentf(format string, args ...any) {
...
func (f *procOrStreamMetaFormatter) format() *genkit.GenKit {
...
func (f *procOrStreamMetaFormatter) formatComment() {
...
func (f *procOrStreamMetaFormatter) formatKV() {
	_, prevLineDiff, prevEOF := f.peekChild(-1)
